...
* [square]#▢# Initialize `ChatLiteLLM` (or equivalent) for model access
* [square]#▢# Initialize `PlaywrightMCPTool` from `langchain-mcp-adapters`
* [square]#▢# Use `create_react_agent` to build the agent instance
* [square]#▢# Create `AgentExecutor` to run the agent
* [square]#▢# Implement `run_chat_loop` using `AgentExecutor.invoke` (or `astream`)
* [square]#▢# Implement `send` method using `AgentExecutor.invoke` (or `astream`)
* [square]#▢# Ensure streaming output is properly configured and handled
